...
def test_codex_does_not_emit_the_removed_web_search_boolean_override():
    # Codex no longer accepts tools.web_search=false. Leave launch unchanged;
    # callers that need the canonical policy must use web_search="disabled".
    from benchflow.agents.registry import AGENTS

    base_cmd = AGENTS["codex-acp"].launch_cmd
    assert _agent_launch_with_web_policy("codex-acp", disallow=False) == base_cmd
    assert _agent_launch_with_web_policy("codex-acp", disallow=True) == base_cmd
    assert "tools.web_search=false" not in base_cmd
